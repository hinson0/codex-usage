...
- This feature releases as `0.4.3 (7)` after publication validation added draft-visibility polling.
...
- Produces: version `0.4.3 (7)`, GitHub secret `SPARKLE_PRIVATE_KEY`, enabled future automatic releases, and public release `v0.4.3`.

- [ ] **Step 1: Make the packaging version assertion fail**

Change only `Tests/PackagingTests.sh` to expect short version `0.4.3` and build
number `7`.

Run: `Tests/PackagingTests.sh`

Expected: FAIL because the previous validation commit still contains `0.4.2 (6)`.

- [ ] **Step 2: Bump the single version source and update docs**

Change the two plist values to `0.4.3` and `7`. Update both READMEs with:
...
Expected: packaging checks pass for `0.4.3 (7)`.
...
Expected: tag `v0.4.3` and one published GitHub Release are created at the
...
```bash
gh release view v0.4.3 --repo hinson0/codex-usage --json tagName,isDraft,isPrerelease,url,assets
...
`CodexUsage-v0.4.3-macOS.zip` under immutable tag `v0.4.3`.
...
affects only the popover, and checking on `0.4.3` reports no newer update.
