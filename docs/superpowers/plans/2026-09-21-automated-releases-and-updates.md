...
- This feature releases as `0.4.2 (6)` after validation also upgraded the deprecated checkout runtime.
...
- Produces: version `0.4.2 (6)`, GitHub secret `SPARKLE_PRIVATE_KEY`, enabled future automatic releases, and public release `v0.4.2`.

- [ ] **Step 1: Make the packaging version assertion fail**

Change only `Tests/PackagingTests.sh` to expect short version `0.4.2` and build
number `6`.

Run: `Tests/PackagingTests.sh`

Expected: FAIL because the previous validation commit still contains `0.4.1 (5)`.

- [ ] **Step 2: Bump the single version source and update docs**

Change the two plist values to `0.4.2` and `6`. Update both READMEs with:
...
Expected: packaging checks pass for `0.4.2 (6)`.
...
Expected: tag `v0.4.2` and one published GitHub Release are created at the
...
```bash
gh release view v0.4.2 --repo hinson0/codex-usage --json tagName,isDraft,isPrerelease,url,assets
...
`CodexUsage-v0.4.2-macOS.zip` under immutable tag `v0.4.2`.
...
affects only the popover, and checking on `0.4.2` reports no newer update.
